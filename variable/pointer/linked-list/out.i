# 1 "ex_s1.1_single_linked_list.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 418 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "ex_s1.1_single_linked_list.c" 2
# 18 "ex_s1.1_single_linked_list.c"
...
# 19 "ex_s1.1_single_linked_list.c" 2
...
# 20 "ex_s1.1_single_linked_list.c" 2
...
# 21 "ex_s1.1_single_linked_list.c" 2
...
void delete(LinkedList *list, Node *node) {
  if (node == list->head) {
    ...
    Node *tmp = list->head;
    while (tmp != ((void *)0) && tmp->next != node) {
      tmp = tmp->next;
    }
    if (tmp != ((void *)0)) {
      tmp->next = node->next;
...
int main() {
  ...
  __builtin___strcpy_chk (samuel->name, "Samuel", __builtin_object_size (samuel->name, 2 > 1 ? 1 : 0));
  samuel->age = 32;

  Employee *sally = (Employee *)malloc(sizeof(Employee));
  __builtin___strcpy_chk (sally->name, "Sally", __builtin_object_size (sally->name, 2 > 1 ? 1 : 0));
  sally->age = 28;

  Employee *susan = (Employee *)malloc(sizeof(Employee));
  __builtin___strcpy_chk (susan->name, "Susan", __builtin_object_size (susan->name, 2 > 1 ? 1 : 0));
  susan->age = 45;
